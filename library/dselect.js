...
function dselect(el, option = {}) {
  ...
  function createDom() {
    ...
    <div class="dropdown ${classElement} ${additionalClass}">
      <button class="${classToggler} ${!el.multiple && clearable ? classTogglerClearable : ''}" data-dselect-text="${!el.multiple && selectedText(el.options)}" type="button" data-bs-toggle="dropdown" aria-expanded="false"${autoclose} style="padding:0.3em; margin-bottom: 0.5em; padding-left: 0.5em; text-align: left; font-size: 14px">
        <span>${selectedTag(el.options, el.multiple)}</span>
      </button>
